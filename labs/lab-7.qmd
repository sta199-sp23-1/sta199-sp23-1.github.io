...
c.  Another type of plot that is helpful for visualizing distributions of continuous variables (like `weight`) across levels of a categorical treatment (like `feed`) is called a *violin plot*. A violin plot represents each distribution with a smooth density estimate.
...
b.  Suppose we would like to compare all pairs of feed groups to test for differences in expected weight.
    There are 6 types of feed, so there are ${6 \choose{2}} = 6*5/2 = 15$ pairs we could compare.
